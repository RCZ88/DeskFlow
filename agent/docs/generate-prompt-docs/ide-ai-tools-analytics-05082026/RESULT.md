...
export function repairDoubleEncodedUtf8(str: string): string {
 ...
 if (/[Ã¢Â€Â¦éçëïôÃ»]/.test(str) || str.includes('â€')) {
...
export function sanitizeMojibake(text: string): string {
 ...
 .replace(/ê/g, 'ê').replace(/ë/g, 'ë').replace(/Ã/g, 'à')
 .replace(/î/g, 'î').replace(/ï/g, 'ï').replace(/ô/g, 'ô')
 .replace(/ö/g, 'ö').replace(/ü/g, 'ü').replace(/ç/g, 'ç');
}
```
...
### Feature C (Mojibake)
- [ ] **DB:** Run app. Check `workspace_problems` table -> verify `â€"` is now `—` and `é` is `é`.
