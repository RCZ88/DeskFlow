/* ============================================================================
 * SkyDome.tsx — cheap gradient night sky + faint procedural stars, replacing the
 * flat background color. One inward-facing sphere, one shader, no textures.
 * `stars` toggles the star field (off on the Low graphics preset).
 * ========================================================================== */
...
export function SkyDome({ radius = 1600, stars = true }: { radius?: number; stars?: boolean }) {
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uHorizon: { value: new THREE.Color('#161a3a') },
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        col += uGlow * pow(1.0 - h, 6.0) * 0.35;
        vec2 g = floor(vDir.xz * 220.0);
        float s = step(0.9975, hash(g)) * smoothstep(0.1, 0.5, vDir.y) * uStars;
        col += vec3(s) * 0.6;
