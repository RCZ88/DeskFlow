// src/components/finance/FollowThroughCard.tsx
//
// Overview "Follow Through" section (replaces the old OnBehalfOf card).
// Shows: total this month, MoM change, "You'll be repaid", breakdown by label,
// mini trend. Accent amber-400. Renders nothing-heavy empty state when zero.
//
// Props are already-computed values from OverviewTab (keeps the widget dumb).
...
export interface FollowThroughCardProps {
  ...
  momChangePct: number | null; // + = more than last month
  receivable: number; // outstanding "you'll be repaid"
  breakdown: FollowThroughBreakdown[];
  trend: number[]; // last N months, oldest→newest
}
