...
interface MemoriesPageProps {
  /** When true, renders without its own PageShell so it can live as a tab
   * inside LifePage. */
  embedded?: boolean;
}
...
export default function MemoriesPage({ embedded = false }: MemoriesPageProps = {}) {
  ...
  // The reel highlights on-this-day first (if any), then the most recent moments.
  ...
  // Recap plays oldest -> newest so it reads like a story of your life.
  ...
  const body = (
    ...
    >
      ...
      {/* SIGNATURE HERO: film-strip reel + Recap Player entry point */}
