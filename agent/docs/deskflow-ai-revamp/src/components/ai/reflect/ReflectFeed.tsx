 ...
 */
export function ReflectFeed({ state, days, errorMessage, onRetry }: ReflectFeedProps) {
	const [filter, setFilter] = useState<Filter>("all")
	const m = useMotionProps()

	const filtered = useMemo(() => {
		if (filter === "reviewed") return days.filter((d) => Boolean(d.reviewSummary))
		if (filter === "productive") return days.filter((d) => dayStats(d).pct >= 0.6)
		return days
	}, [days, filter])
