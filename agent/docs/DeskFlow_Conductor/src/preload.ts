...
contextBridge.exposeInMainWorld('deskflowAPI', {
  ...
  buildInsightRollup: (date: string) => ipcRenderer.invoke('insights:build-rollup', { date }),
  ...
  financeCryptoCacheStatus: () => ipcRenderer.invoke('finance:crypto-cache-status'),

  // Universal asset prices (stocks, ETFs, commodities, indices, crypto)
  financeFetchAssetPrices: (symbols: string[], assetType?: string, currency?: string) => ipcRenderer.invoke('finance:fetch-asset-prices', symbols, assetType || 'stock', currency || 'usd'),
  financeGetAssetHistory: (symbol: string, assetType?: string, days?: number) => ipcRenderer.invoke('finance:get-asset-history', symbol, assetType || 'stock', days || 30),
  ...
  financeRecalculateBalances: (walletId?: number) => ipcRenderer.invoke('finance:recalculate-balances', walletId),
  ...
  learnGetProfile: (key: string) => ipcRenderer.invoke('learn:getProfile', { key }),
  learnSetProfile: (key: string, value: string) => ipcRenderer.invoke('learn:setProfile', { key, value }),
  learnDeleteProfile: (key: string) => ipcRenderer.invoke('learn:deleteProfile', { key }),
  learnGetAllProfile: () => ipcRenderer.invoke('learn:getAllProfile'),

  // ========== Insight Engine (Rewind) ==========
  getDailyFunFact: () => ipcRenderer.invoke('insights:daily-fun-fact'),
  buildInsightRollup: (date: string) => ipcRenderer.invoke('insights:build-rollup', { date }),
  ...
  // ========== Device Management (paired phones) ==========
