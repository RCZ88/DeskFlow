...
// Backs a single day's journal entry (text + optional voice note) for either
// a specific commitment (commitmentId set) or the general day journal
// (commitmentId null). Text lives in localStorage; audio bytes live in
// IndexedDB and are referenced only by id -- see voiceJournal.ts for the
// hard local-only guarantee.
export function useCovenantJournalEntry(commitmentId: string | null, date: string, initialText?: string) {
  ...
  const save = useCallback(() => persist({ text }), [persist, text]);
