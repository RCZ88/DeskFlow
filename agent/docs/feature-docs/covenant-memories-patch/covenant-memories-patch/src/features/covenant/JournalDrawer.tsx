...
// A private, per-day journal: text and/or a voice note. Everything here is
// stored only on this device (localStorage for text, IndexedDB for audio --
// see voiceJournal.ts). Nothing is transcribed or sent anywhere; the privacy
// note in the UI is not just copy, it is an accurate description of the code.
export function JournalDrawer({ commitmentId, date, initialText }: JournalDrawerProps) {
  const {
    text, setText, save,
    isRecording, startRecording, stopRecording,
    voiceUrl, voiceDurationSec, deleteVoice,
  } = useCovenantJournalEntry(commitmentId, date, initialText);
