...
export function SubscriptionModal({ subscription, wallets, displayCurrency, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    if (subscription) {
      ...
      setForm({ ...INITIAL, wallet_id: firstWallet?.id || 0 });
    }
          ...
          {parseFloat(form.price) > 0 && (
            <div className="px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
              ...
              <p className="text-xs text-zinc-300">
                <span className="font-semibold text-white">{formatCurrency(parseFloat(form.price), displayCurrency)}</span>
                ...
                {form.billing_cycle !== 'monthly' && (
                  <span className="text-zinc-600"> · <span className="text-zinc-500">{formatCurrency(parseFloat(form.price) / (form.billing_cycle === 'yearly' ? 12 : form.billing_cycle === 'quarterly' ? 3 : form.billing_cycle === 'weekly' ? 0.231 : form.billing_interval), displayCurrency)}</span>/mo</span>
                )}
              </p>
            </div>
          )}
