...
// SIGNATURE HERO (signature-design skill): "Constellation of Practice".
// Concept essence -> grace-based consistency: nothing you've done is ever lost.
// Every unique practice day becomes a permanent warm star; a missed day simply
// adds no new star (nothing is removed), and the current streak is a glowing
// thread through the most recent consecutive stars. Milestones send one shooting
// star. This amplifies the focal data (total practice + current streak) instead
// of decorating a corner.
//
// Engineering follows references/motion-engineering.md: canvas capped at DPR 2,
// a delta-time rAF loop that pauses off-screen / in background tabs, only
// transform/opacity-equivalent canvas compositing, and a settled static frame
// under prefers-reduced-motion.

interface ConstellationHeroProps {
  /** Unique YYYY-MM-DD dates the user practiced anything. Order-independent. */
  dates: string[];
  /** Fires a one-shot shooting star when it changes to a truthy value. */
...
const STAR_WARM = [
  [247, 243, 238], // glow white
  [240, 168, 146], // clay light
  [251, 191, 36], // amber
  [232, 134, 107], // clay
];
...
// Tail run of consecutive calendar days ending at the most recent date.
...
export function ConstellationHero({ dates, milestone, children, className = '', height = 172 }: ConstellationHeroProps) {
  ...
  const stars = useMemo<Star[]>(() => {
    ...
    return sorted.map((date, i) => {
      ...
      // x reads left(older) -> right(newer) like a gentle timeline, with jitter
  ...
  useEffect(() => {
    ...
    // expose shooting-star spawn for milestone effect
  ...
  // Fire a shooting star when milestone changes to a truthy value.
  useEffect(() => {
  ...
  const tipStyle = hover ? { left: Math.min(hover.x + 12, 240), top: Math.max(hover.y - 8, 4) } : undefined;
