    ...
    <GlassCard accent="emerald">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {saveLabel && (
            <span className={`text-[10px] text-zinc-500 transition-all duration-300 ${saveState === 'saved' ? 'text-emerald-400' : saveState === 'saving' ? 'text-amber-400' : 'text-zinc-500'}`}>{saveLabel}</span>
          )}
          <Button
            variant="ghost"
            size="sm"
      ...
      {error && (
        <div className="rounded-lg bg-red-500/8 ring-1 ring-red-500/15 p-3 text-xs text-red-400 mb-3">
          {error}
        </div>
      )}
