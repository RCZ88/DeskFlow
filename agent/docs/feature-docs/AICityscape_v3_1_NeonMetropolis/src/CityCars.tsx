/* ============================================================================
 * CityCars.tsx — v3.1 ORBIT traffic. Because the city is now radial, cars simply
 * ORBIT the center on the ring roads (plus a few run along radial spokes). This
 * removes ALL the old pathing bugs (flipped / upside-down / stuck cars) because
 * motion is pure angular math — the result is clean glowing rings of traffic.
 *
 * Two instanced meshes: dark car bodies + additive underglow planes (radial
 * glow generated in code). Bloom turns the underglow into cyberpunk neon.
 * ========================================================================== */
...
const GLOW_FLAT = new THREE.Euler(-Math.PI / 2, 0, 0) // radial glow is symmetric
...
export function CityCars({
  ...
  const cars = useMemo<Car[]>(() => {
    ...
    for (let i = 0; i < nOrbit; i++) {
      // pick a ring road, offset slightly for a 2-lane feel, random direction
  ...
  useFrame((_, dt) => {
    ...
    cars.forEach((c, i) => {
      ...
      // underglow flat on the road (symmetric -> no heading needed)
