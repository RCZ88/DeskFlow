...
import { ProductivityFocusZone } from './dashboard/ProductivityFocusZone';
...
import {
  ...
  BarChart3, Sparkles, Play,
  Wallet, GraduationCap, ExternalLink, BarChart
} from 'lucide-react';
...
export default function DashboardPage({
  ...
  useEffect(() => {
    ...
    if (!api?.getHomeSummary) return;
    api.getHomeSummary().then(setHomeSummary).catch(() => {});
  }, []);
  ...
  // Periodic foreground refresh (every 60s) — recovers from missed foreground-changed events
  useEffect(() => {
...
window.deskflowAPI.onBrowserTrackingEvent((data: any) => {
  ...
  // Stopwatch — resets on tier transitions per timerBehavior settings
  useEffect(() => {
    // Determine current tier
    const tb = timerBehaviorRef.current;
    ...
    // Tier transition: reset stopwatch based on timerBehavior
    if (prevTier && prevTier !== tier) {
      const shouldReset =
        tier === 'productive' ||
        (tier === 'distracting' && tb.distractingAction === 'reset') ||
        (tier === 'neutral' && tb.neutralAction === 'reset');

      if (shouldReset) {
        console.log(`[Dashboard] Stopwatch: RESET (${prevTier} → ${tier})`);
    ...
    // Track accumulation only for tiers that should be counted per settings
    const isTrackingTier = tier === 'productive' ||
      (tier === 'distracting' && tb.distractingAction === 'ignore') ||
      (tier === 'neutral' && tb.neutralAction === 'ignore');
    
    const shouldAccumulate = !isPaused && (hasRealApp || isExternal) && isTrackingTier;
    const shouldPause = isPaused || (!isTrackingTier && hasRealApp && !isExternal);
    ...
    if (shouldAccumulate || isExternal) {
      console.log(`[Dashboard] Stopwatch: timer RUNNING (tier: ${tier}, accumulated: ${Math.floor(stopwatchAccumulatedRef.current / 1000)}s)`);
    }
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <HeroBand
              ...
              goalCurrent={homeSummary.data?.focusMinutes ?? (stats.productiveMinutes || 0)}
              focusActive={deepFocus.state.active}
            />
          </motion.div>
          ...
          <ProductivityFocusZone
            focusState={deepFocus.state}
            focusHistory={deepFocus.history}
            focusRankings=
              todayBest: sessionsData.stats.todayBest,
              todayTotal: sessionsData.stats.todayTotal,
              weekBest: sessionsData.stats.weekBest,
              weekTotal: sessionsData.stats.weekTotal,
              allTimeBest: sessionsData.stats.allTimeBest,
            
            onOpenHeatmap={() => setExpandedModal('heatmap')}
            onOpenOrbit={() => setExpandedModal('solar')}
            onStartFocus={deepFocus.start}
            onEndFocus={deepFocus.end}
          />
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-semibold uppercase tracking-wider text-zinc-500">At a Glance</h2>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {/* Activity Card */}
              <SummaryCard
                title="Activity"
                value={homeSummary ? `${Math.round(homeSummary.focusMinutes)}min focus` : `${Math.round(stats.productiveMinutes || 0)}min focus`}
                subtitle={`${stats.productivePercent || 0}% productive`}
              ...
              <SummaryCard
                title="Finance"
                value={homeSummary?.financeLocked ? '••••' : homeSummary ? `$${homeSummary.totalBalance.toLocaleString()}` : '—'}
                subtitle={homeSummary ? `${homeSummary.walletCount} wallet${homeSummary.walletCount !== 1 ? 's' : ''}` : '—'}
                icon={<Wallet className="w-4 h-4 text-amber-400" />}
                accentColor="from-amber-500/10 to-amber-600/5"
                onClick={() => navigate('/finance')}
                masked={homeSummary?.financeLocked}
              />
              {/* Learn Card */}
              <SummaryCard
                title="Learn"
                value={homeSummary ? `${homeSummary.dueReviews} due` : '—'}
                subtitle={homeSummary && homeSummary.dueReviews > 0 ? 'Ready for review' : 'All caught up'}
                icon={<GraduationCap className="w-4 h-4 text-indigo-400" />}
                accentColor="from-indigo-500/10 to-indigo-600/5"
                onClick={() => navigate('/learn')}
              />
              {/* External Card */}
              <SummaryCard
                title="External"
                value={homeSummary ? `${Math.round(homeSummary.sleepSeconds / 60)}min sleep` : '—'}
                subtitle={homeSummary ? 'Last night' : 'No data'}
          ...
          {/* ══════════════════════════════════════════════════════════════
                BAND 3: Rewind Entry + Recent Context
                ══════════════════════════════════════════════════════════════ */}
