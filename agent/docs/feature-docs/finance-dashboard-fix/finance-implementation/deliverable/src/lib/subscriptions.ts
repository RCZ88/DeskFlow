...
// Pure helpers for subscription math + ledger linkage convention.
// No schema change: linkage is by description + a `sub:{id}` tag string.
import type { FinanceSubscription } from "../components/finance/finance-types";

const PER_YEAR: Record<FinanceSubscription["billing_cycle"], number> = {
  weekly: 52,
  monthly: 12,
  quarterly: 4,
  yearly: 1,
