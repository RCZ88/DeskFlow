...
**Since v1 you did the big IA move (Doc 1 C2): the tracking merge shipped.** `pages/ActivityPage.tsx` now renders a tabbed shell (Apps / Websites / Productivity) that lazy-loads the existing `StatsPage`, `BrowserActivityPage`, and `ProductivityPage` as embedded panels, hoists shared props (period/offset/tiers), and syncs the active tab to `?tab=` in the URL. This is almost exactly the C3 plan — nicely done. Remaining C3 follow-ups: add the old-route redirects (`/stats`, `/browser`, `/productivity` -> `/activity?tab=`) and confirm the shared data is fetched once at the shell, not re-fetched inside each panel.

**Two new modules validate the target architecture.** `services/learn/*` and `components/finance/*` are exactly the domain-module shape Doc 1 A1 asks `main.ts` to become: cohesive folders, typed contracts (`shared/learn/types.ts`, `components/finance/finance-types.ts`), SQLite persistence via IPC, and (for Learn) a migrations file. Use these as the reference pattern when you carve `main.ts` — you already know how to do it, you just haven't backfilled the old code.

**God-objects grew, not shrank.** `main.ts` 21,432 -> **22,043**; `DashboardPage.tsx` -> **3,280** (43 useState / 34 useEffect); `IDEProjectsPage.tsx` **5,434**; `TerminalPage.tsx` **5,315**. The A1/A2 findings stand and are more urgent. New watch item: `components/learn/LearnPage.tsx` (981 lines, ~30 useState) is on the same trajectory — split it now (Doc 7 L4).

**Repo hygiene regressed.** Still ~28 `.bak`/`.backup` files, and now **new** committed archives: `components/finance/finance.zip`, `pages/FinancePage.backup.tsx`, plus `AiPage.tsx.bak.*` and service `.bak` files. B1 stands — delete them and add `*.bak`, `*.backup.*`, `*.zip` to `.gitignore`.

**Typed-bridge inconsistency spread.** The `(window as any).deskflowAPI` anti-pattern (Doc 1 D) is now also in `components/learn/LearnPage.tsx:36`. Standardize on the typed bridge — especially painful here because the Learn backend is fully typed and the renderer throws that away.

**Still open from v1:** `insights/` domain extraction, `TimeRangeContext`/`TrackingDataContext`, the `data:`-URL terminal window, and unifying the two data-loading paradigms.
