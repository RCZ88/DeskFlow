...
import { Monitor, Globe, Target, Activity } from 'lucide-react';
...
export default function ActivityPage(props: ActivityPageProps) {
  const [activeTab, setActiveTab] = useState<TabKey>(() => {
    try {
      ...
      if (tab === 'websites' || tab === 'productivity') return tab;
    } catch {}
    return 'apps';
  });
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="sticky top-0 z-30 -mx-5 px-5 bg-zinc-900/20 backdrop-blur-md border-b border-zinc-800/50">
        <div className="flex items-center gap-1 py-2">
          <div className="h-9 w-9 rounded-xl grid place-items-center mr-2" style={{ background: `${activeConfig.accent}22` }}>
            <Activity className="w-5 h-5" style={{ color: activeConfig.accent }} />
          </div>
