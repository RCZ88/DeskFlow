...
export function FinancePage() {
  ...
  const baseCurrency = useMemo(() => {
    const active = accounts.find(a => !a.is_archived);
  ...
  const handleSetRememberDevice = async (remember: boolean, days: number) => {
    if (window.deskflowAPI) {
      ...
      setSecuritySettings(prev => ({ ...prev, rememberDevice: remember, rememberDeviceExpiry: remember ? Date.now() + (days * 24 * 60 * 60 * 1000) : null }));
    }
  };
  ...
  const handleSetLockTimeout = async (timeoutMs: number) => {
    if (window.deskflowAPI) {
      ...
      setSecuritySettings(prev => ({ ...prev, lockTimeout: timeoutMs }));
      resetLockTimer();
    }
  };
  ...
  if (isLocked && !isFirstTime) {
    return (
      <div className="flex flex-col h-full" style={{ ['--page-accent' as string]: '#10b981' }}>
        <FinanceStickyHeader isLocked={true} netWorth={0} displayCurrency={displayCurrency} onToggleLock={() => {}} />
        <FinanceLockScreen
    ...
    <PageShell page="finance" variant="sticky-header" style={{ ['--page-accent' as string]: '#10b981' }}>
      <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 shrink-0">
        ...
        <div className="flex items-center gap-2">
            ...
            <AnimatePresence>
              {showCurrencyPicker && (
                <motion.div
                  ...
                  className="absolute top-full right-0 mt-1 w-36 max-h-48 overflow-y-auto bg-zinc-800 border border-zinc-700 rounded-lg z-10 shadow-xl"
                >
                {['USD', 'IDR', 'SGD', 'GBP', 'EUR', 'JPY', 'AUD', 'CNY', 'KRW', 'INR'].map(code => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-zinc-700 transition-colors ${
                      displayCurrency === code ? 'text-emerald-400 bg-emerald-500/10' : 'text-zinc-300'
                    }`}
                  >
          ...
          <button
            onClick={handleLock}
            className="px-3 py-1.5 min-h-[44px] min-w-[44px] rounded-lg bg-zinc-800 text-zinc-400 hover:text-zinc-200 text-xs font-medium transition-colors border border-zinc-700/30 focus-visible:ring-2 ring-emerald-500/50 ring-offset-2 ring-offset-zinc-950"
            title="Lock finance"
          >
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 w-12 h-12 rounded-full bg-emerald-500 hover:bg-emerald-400 text-white flex items-center justify-center transition-colors z-[var(--z-elevated)] focus-visible:ring-2 ring-emerald-500/50 ring-offset-2 ring-offset-zinc-950"
