...
export function VisualizerView() {
  ...
  const [showSafeZones, setShowSafeZones] = useState(ui.showSafeZones)
  ...
  if (!overlayPlan?.overlays?.length && !activeSession.scenePlan?.overlays?.length) return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center">
      <FileJson size={28} className="mb-3 text-zinc-600" />
      <p className="text-[13px] font-medium text-zinc-400">No overlay plan yet</p>
      <p className="text-[11px] text-zinc-500 mt-1 mb-4">Generate an overlay plan to preview.</p>
      ...
      <BlurFade>
        <div className="flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowSafeZones(!showSafeZones)} className={`studio-btn rounded-lg text-[11px] px-3 py-1.5 transition-colors ${showSafeZones ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/30' : 'bg-zinc-800/50 text-zinc-500 border border-zinc-700/30'}`}>Safe Zones</button>
        ...
        <BlurFade delay={0.15}>
          <div className="relative rounded-xl overflow-hidden border border-zinc-700/50 bg-[#0D1117]" style={{ width: 270, height: 480 }}>
            ...
            {activeOverlays.map((o: any, i: number) => {
              const color = o.type === 'hook' ? '#fbbf24' : o.type === 'caption' ? '#94a3b8' : '#e2e8f0'
              return (
