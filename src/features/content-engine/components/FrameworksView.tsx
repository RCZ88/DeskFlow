...
export function FrameworksView() {
  ...
  return (
    <section className="space-y-6 p-6">
      <SectionHeader label="Content Engine / 07" title="Frameworks" icon={<Layers size={14} className="text-white/70" />} action={<Chip>Versioned prompt frameworks</Chip>} />

      {loading && <LoadingBlock label="Loading frameworks…" />}
      {error && <ErrorState message={error} onRetry={load} />}

      {!loading && !error && frameworks.length === 0 && (
        <EmptyState icon={<Layers size={28} />} title="No frameworks yet" hint="Frameworks package your winning rules into reusable, versioned prompts for script generation." />
      )}
