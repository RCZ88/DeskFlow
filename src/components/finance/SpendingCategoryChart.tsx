...
const FT_SHADES = ["#f59e0b", "#d97706", "#b45309", "#92400e", "#78350f"];
...
export default function SpendingCategoryChart({ data, baseCurrency, displayCurrency, convertAmount: convert, allTransactions = [] }: Props) {
  const chartData = useMemo(() => {
    ...
    data.forEach((item, idx) => {
      ...
      values.push(convert(item.amount, baseCurrency, displayCurrency));
    ...
    if (ftExpenses.length > 0) {
      ...
      ftExpenses.forEach(t => {
        const label = t.on_behalf_of_label || t.description || "Follow Through";
        ftByLabel[label] = (ftByLabel[label] || 0) + Math.abs(t.amount);
      });
      Object.entries(ftByLabel).forEach(([name, amount], idx) => {
        labels.push(`${name} (FT)`);
        values.push(convert(amount, baseCurrency, displayCurrency));
  ...
  if (chartData.data.length === 0) {
    return (
      <div className="rounded-xl border border-zinc-700/30 p-5">
        <div className="flex items-center gap-2 mb-4"><PieChart className="w-4 h-4 text-zinc-500" /><h3 className="text-sm font-semibold text-white">Spending by Category</h3></div>
        <div className="flex flex-col items-center justify-center py-10 text-zinc-500"><PieChart className="w-12 h-12 mb-3 opacity-30" /><p className="text-sm">No spending data yet</p></div>
      </div>
    );
  }
      ...
      <div className="flex gap-4 items-start">
        <div className="relative w-[180px] h-[180px] shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-lg font-bold text-white">{fmtCurrency(totalSpent, displayCurrency)}</span>
          ...
          {chartData.labels.map((label, idx) => {
            ...
            return (
              <div key={label} className="flex items-center gap-2 text-xs">
                <span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: chartData.bgColors[idx] }} />
                <span className={`flex-1 truncate ${isFt ? "text-amber-400" : "text-zinc-300"}`}>{label}</span>
