...
import { Plus, ArrowUpRight, ArrowDownRight, ArrowLeftRight, Search, Trash2, Lock as LockIcon } from 'lucide-react';
...
export function TransactionsTab({ transactions, accounts, categories, wallets, loading, error, onRetry, displayCurrency, baseCurrency, onAddTransaction, onDeleteTransaction, onVerifyPassword }: TransactionsTabProps) {
  ...
  return (
    <div className="p-5 space-y-3">
      ...
      <div className="sticky top-14 z-[var(--z-elevated)] bg-zinc-950/90 backdrop-blur-lg border-b border-zinc-800/60 py-3 -mx-5 px-5 mb-4">
          ...
          <div className="flex gap-1">
            {(['all', 'income', 'expense', 'transfer'] as const).map(t => (
              <button
                ...
                className={`relative px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-colors capitalize focus-visible:ring-2 ring-emerald-500/50 ring-offset-2 ring-offset-zinc-950 ${
                  typeFilter === t
                    ? 'text-emerald-400'
        ...
        <div className="flex items-center gap-2 mt-2">
          <input
            ...
            className="flex-1 bg-zinc-800/80 border border-zinc-700/50 rounded-lg px-2 py-1.5 text-[11px] text-zinc-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus-visible:ring-2 ring-emerald-500/50 ring-offset-2 ring-offset-zinc-950"
          />
          ...
          {(dateStart || dateEnd) && (
            <button
              ...
              className="px-2 py-1.5 rounded text-[10px] text-zinc-500 hover:text-zinc-300 transition-colors focus-visible:ring-2 ring-emerald-500/50 ring-offset-2 ring-offset-zinc-950"
            >
              Clear
            </button>
          )}
                  ...
                  return (
                    <GlassSurface
                      ...
                      className={`!p-3 hover:bg-zinc-800/70 transition-all group border-l-2 ${
                        isIncome ? 'border-l-emerald-500/50 hover:border-l-emerald-500/70' :
                        isExpense ? 'border-l-red-500/50 hover:border-l-red-500/70' :
                        isTransfer ? 'border-l-amber-500/50 hover:border-l-amber-500/70' :
                        'border-l-zinc-700/50 hover:border-l-zinc-500/70'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <p className={`text-sm font-semibold tabular-nums shrink-0 ${
                          ...
                          {isIncome ? '+' : isExpense ? '-' : ''}{showNumbers ? fc(Math.abs(txn.amount)) : maskNumber(fc(Math.abs(txn.amount)), maskMode, maskFixedValue)}
                        </p>
                              ...
                              {deletePasswordTarget === txn.id ? (
                                <div className="flex items-center gap-1">
                                  <div className="relative">
                                    <input
                                      ...
                                      autoFocus
                                      className="w-24 bg-zinc-800/80 border border-zinc-700/50 rounded px-2 py-1 text-[10px] text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-red-500/50"
                                    />
                                    {deletePasswordError && (
                                ...
                                <div className="flex items-center gap-1">
                                  <button
                                    ...
                                    className="px-2 py-1 rounded text-[10px] font-medium bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors focus-visible:ring-2 ring-emerald-500/50 ring-offset-2 ring-offset-zinc-950"
                                  >
