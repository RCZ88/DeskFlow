...
const RATING_CONFIG: Record<CardRating, { label: string; interval: string; borderColor: string; textColor: string }> = {
  1: { label: 'Again', interval: '1m', borderColor: 'rgba(217,104,70,0.5)', textColor: '#d96846' },
  2: { label: 'Hard', interval: '6d', borderColor: 'rgba(168,162,158,0.5)', textColor: '#a8a29e' },
  3: { label: 'Good', interval: '10d', borderColor: 'rgba(251,191,36,0.5)', textColor: '#fbbf24' },
  4: { label: 'Easy', interval: '24d', borderColor: 'rgba(111,179,143,0.5)', textColor: '#6fb38f' },
};
      ...
      <div className="perspective-[1000px] w-full max-w-[480px] mx-auto h-[280px]">
        ...
        >
          <div className="absolute inset-0 backface-hidden rounded-xl border border-zinc-800 flex items-center justify-center p-6"
            style={{ background: 'rgba(217,104,70,0.06)', backfaceVisibility: 'hidden' }}>
            {renderFront()}
            {!revealed && <div className="absolute bottom-3 text-xs text-zinc-600">Click or Space to flip</div>}
          </div>
          <div className="absolute inset-0 backface-hidden rounded-xl border border-zinc-800 flex items-center justify-center p-6"
            style={{ background: 'rgba(111,179,143,0.06)', backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
            {renderBack()}
          </div>
        </div>
      </div>
