...
import { X, ImageIcon, Copy, Check, Upload, Loader2, ChevronRight, Eye } from 'lucide-react';
...
export function PendingIllustrationsPanel({ lesson, open, onClose, onNavigateToNode }: Props) {
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {pending.map((item) => (
                          <div
                            key={item.blockId}
                            className="rounded-xl border border-amber-500/15 bg-amber-500/[0.03] overflow-hidden"
                          >
                            <div className="px-3 py-2.5 border-b border-amber-500/10">
                              ...
                              {item.concept && (
                                <p className="text-[11px] text-zinc-500 mt-1 truncate">{item.concept}</p>
                              )}
                            </div>
                      ...
                      <div className="space-y-2">
                        {done.map((item) => (
                          <button
                            key={item.blockId}
                            onClick={() => { onNavigateToNode(item.nodeId); onClose(); }}
                            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-zinc-800/20 border border-zinc-800/40 hover:bg-zinc-800/40 transition-colors text-left group"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs text-zinc-300 truncate group-hover:text-emerald-300 transition-colors">{item.nodeTitle}</p>
                              {item.concept && (
                                <p className="text-[10px] text-zinc-600 truncate mt-0.5">{item.concept}</p>
                              )}
                            </div>
