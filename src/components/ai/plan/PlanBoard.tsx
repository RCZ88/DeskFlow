...
export function PlanBoard(props: PlanBoardProps) {
	...
	// The parent's goalsDataState is based on DAILY goals, not long-term goals.
	// Compute our own state from the actual long-term goals prop.
	...
	// ADDED — sync from prop when parent updates (e.g., after boot)
	useEffect(() => { setDraft(notes) }, [notes])

	// ADDED — cleanup pending debounce on unmount
	useEffect(() => () => { if (debounceRef.current) clearTimeout(debounceRef.current) }, [])

	// ADDED — debounced auto-save (600ms trailing edge)
