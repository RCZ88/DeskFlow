...
export default function AnalyticsDashboard({ aiUsage, sessions, problems, requests, dailyStats, appStats, promptHistory, loading, period }: {
  aiUsage: any; sessions: any[]; problems: any[]; requests: any[]; dailyStats: any[]; appStats: any[]; promptHistory: any[]; loading: boolean; period: string;
}) {
  ...
  const dailyTrend = useMemo(() => {
    ...
    for (const stat of dailyStats) { const day = stat.day || stat.date; if (!day) continue; dayTotals[day] = (dayTotals[day] || 0) + (stat.total_sec || 0); }
