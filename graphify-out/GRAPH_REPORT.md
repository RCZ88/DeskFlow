...
## Corpus Check
- 300 files · ~4,342,590 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1554 nodes · 2227 edges · 61 communities detected
...
Cohesion: 0.02
Nodes (103): run(), acquireLock(), addLog(), extractJsonFromResponse(), getSavedSkillIds(), handleMCPStdout(), loadAutoAssignConfig(), loadPreferences() (+95 more)
...
Cohesion: 0.04
Nodes (35): AgentHostService, acquireLock(), backfillStatsTables(), buildWeeklyHeatmap(), clearAgentTimeout(), clearTerminalReadyFallback(), detectEditsInOutput(), getLocksForFile() (+27 more)

### Community 3 - "Community 3"
Cohesion: 0.05
Nodes (12): CompactionService, createCompactionService(), DefaultLLMProvider, ContextAssemblyService, createContextAssemblyService(), ensureDb(), withDb(), createProjectContextService() (+4 more)
...
Cohesion: 0.04
Nodes (17): formatTimeFromHours(), getTotalTime(), computeChartDateRange(), getPersistedActivityFeed(), getPersistedTimerState(), handleDayClick(), loadExternalData(), getDateRange() (+9 more)
...
Cohesion: 0.07
Nodes (21): cleanCorruptedData(), clearToday(), confirmSleepDetection(), dismissSleepDetection(), handleActivity(), handleFocus(), handleStorage(), handleStorageChange() (+13 more)
...
Cohesion: 0.16
Nodes (7): loadThread(), AIService, callOpenRouter(), cleanAIJson(), parseAIJson(), loadCompleted(), TutorialProvider()
...
- **Thin community `Community 27`** (11 nodes): `WalletDetailView.tsx`, `getDenominations()`, `handleAddAsset()`, `handleAddLink()`, `handleRemoveAsset()`, `handleRemoveLink()`, `handleSave()`, `maskField()`, `quickAdd()`, `renderDetailBody()`, `updateCount()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (9 nodes): `defaultConfig()`, `handleClearCache()`, `handleRefresh()`, `handleSave()`, `handleStop()`, `handleTestStart()`, `loadCurrentStatus()`, `updateSource()`, `LibraryConfigModal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 55`** (5 nodes): `addTag()`, `handleSave()`, `handleTagKeyDown()`, `removeTag()`, `SessionEditDialog.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 58`** (4 nodes): `preload.ts`, `preload.mjs`, `handler()`, `preload.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 59`** (4 nodes): `AfkPromptModal()`, `formatElapsed()`, `formatTime()`, `AfkPromptModal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 69`** (4 nodes): `TasteKnobs.tsx`, `getAesthetic()`, `SliderRow()`, `TasteKnobs()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 70`** (4 nodes): `useStableMemo.ts`, `deepEqual()`, `shallowEqual()`, `useStableMemo()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 71`** (3 nodes): `feature_inventory.py`, `print_summary()`, `Print a summary of all features and their status.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 72`** (3 nodes): `QuickAddModal.tsx`, `handleSave()`, `QuickAddModal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 73`** (3 nodes): `parseBlocks.legacy.js`, `parseBlocks()`, `parseBlocks.legacy.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 74`** (3 nodes): `planningParser.js`, `parseChecklist()`, `planningParser.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 75`** (3 nodes): `buildPrompt()`, `handleSend()`, `DSLGenerationModal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 77`** (3 nodes): `groupSteps()`, `InitializeProgressModal()`, `InitializeProgressModal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 85`** (3 nodes): `generateAllSpecsMarkdown()`, `generateMarkdown()`, `feature-specs.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 86`** (3 nodes): `applyFilter()`, `sortAndGroupChecks()`, `checklistAlgorithm.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (2 nodes): `graphify-pipeline.py`, `Graphify Pipeline for Windows Runs the full knowledge graph extraction pipeline`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `getCurrencyInfo()` connect `Community 5` to `Community 2`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `setHours()` connect `Community 6` to `Community 0`, `Community 1`, `Community 10`, `Community 14`?**
